...
curl -X PUT -d "@/var/www/html/current_conditions.json" -H "x-ms-date: $(TZ=GMT date '+%a, %d %h %Y %H:%M:%S %Z')" -H "x-ms-blob-type: BlockBlob" "https://weatherman.blob.core.windows.net/%24web/current_conditions.json${SAS_TOKEN}"
